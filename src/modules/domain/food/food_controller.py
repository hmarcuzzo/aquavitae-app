...
from src.modules.domain.food.dto.food_query_dto import FindAllFoodQueryDto, OrderByFoodQueryDto
...
async def create_food(request: CreateFoodDto, database: Session = Depends(get_db)) -> Optional[FoodDto]:
...
async def get_all_food_paginated(
        pagination: FindManyOptions = Depends(GetPagination(Food, FindAllFoodQueryDto, OrderByFoodQueryDto)),
...
async def get_food_by_id(id: str, database: Session = Depends(get_db)) -> Optional[FoodDto]:
