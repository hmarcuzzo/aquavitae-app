...
from src.modules.domain.meal.interfaces.food_can_eat_at_interface import FoodCanEatAtInterface
...
async def import_default_types_of_meal(df: pd.DataFrame) -> dict:
    ...
    system_types_of_meal["T"] = []
    for key in system_types_of_meal.keys():
        if key != "T":
            system_types_of_meal["T"] += system_types_of_meal[key]
...
async def import_default_types_of_meal_and_can_eat_at(
    ...
    can_eat_at_sheet_name: str = "can_eat_at",
):
    ...
    can_eat_at_sheet_name = (
        input(f"Can Eat At Sheet name [{can_eat_at_sheet_name}]: ") or can_eat_at_sheet_name
    )
    ...
    can_eat_at_df = pd.read_excel(
        ROOT_DIR + os.path.join(file_name), sheet_name=can_eat_at_sheet_name, header=0
    )
