    ...
    def __call__(
        ...
        if sort:
            ...
            for sort_param in sort:
                ...
                pagination_params["sort"].append(
                    PaginationSort(field=sort_param_split[0], by=sort_param_split[1])
                )

            if self.order_by_query and not PaginationUtils.validateSortFilter(
                pagination_params["sort"], self.order_by_query
            ):
        ...
        if search:
            ...
            if self.find_all_query and not PaginationUtils.validateSearchFilter(
                pagination_params["search"], self.find_all_query
            ):
